...
data/                personal fleet records; LOCAL, gitignored as a whole
  ...
  <id>/binding       opt-in task-contract binding record for that task; written only by bin/fm-task-contract.sh adopt
...
When every candidate is tight, preserve the task's strongest reasoning class rather than silently downgrading it solely to conserve quota; stop and report the tight choice if that class cannot proceed.
...
Run `bin/fm-dispatch-resolve.sh` directly on the written brief in the same turn, with no preflight, and on `clear` apply `task-contract` suitability before passing its `profile:` line to `fm-spawn` or stating a reason to override; `ambiguous`, `escalate`, `error`, and off all mean the intake above, unchanged (contract: `docs/configuration.md` "Typed dispatch resolution").
...
Load `task-contract` before finalizing the brief, then fill `## Firstmate spec` (`{FIRSTMATE_SPEC}`) with the compact suitability basis and only the build instructions that ask requires, naming what stays out of scope when the ask is narrow; a generalization, consistency sweep, or extra hardening the captain did not ask for is follow-up work to note, not scope to add.
For coupled, unresolved, authority-limited, or evidence-heavy tasks, use `task-contract`'s proportionate handoff and pre-validation self-check guidance in `## Firstmate spec`; do not weaken the selected delivery path.
...
- `task-contract` - load before finalizing or materially revising a ship specification or scout question, and before revising a coupled, unresolved, authority-limited, or evidence-heavy handoff for validation; it owns semantic risk, specification completeness, high-risk handoff, pre-validation self-check guidance, and suitability input to existing model-fit selection.
