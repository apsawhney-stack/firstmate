...
For coupled, unresolved, authority-conflicting, or evidence-heavy tasks, use `task-contract`'s proportionate handoff and pre-validation self-check guidance in `## Firstmate spec`; do not weaken the selected delivery path.
...
- `task-contract` - load before finalizing or materially revising a ship specification or scout question, and before revising a coupled, unresolved, or evidence-heavy handoff for validation; it owns semantic risk, specification completeness, high-risk handoff, pre-validation self-review guidance, and suitability input to existing model-fit selection.
